...
// Itens iniciais (gratis) que o user ja possui ao abrir o app a primeira vez
const DEFAULT_OWNED = [
...
export function useInventory(): InventoryApi {
    ...
    (async () => {
      try {
        ...
        if (raw) {
          ...
          // garante itens default sempre owned (caso user tenha tipo gratis sem comprar)
  ...
  const equip = useCallback(
    (id: string) => {
      ...
      if (!item) return;
      if (!owned.has(id)) return; // so equipa o que possui
