...
function key(p: Position): string { return `${p.x}:${p.y}`; }
function heuristic(a: Position, b: Position): number { return Math.abs(a.x - b.x) + Math.abs(a.y - b.y); }
...
export function findPath(map: BeeHouseMap, start: Position, goal: Position): Position[] {
  ...
  while (open.size > 0) {
    ...
    if (!current) break;
    if (current.position.x === goal.x && current.position.y === goal.y) return reconstruct(current);
    ...
    for (const offset of NEIGHBORS) {
      ...
      if (!existing || tentativeG < existing.g) {
        ...
        open.set(nextKey, { position: next, g: tentativeG, h, f: tentativeG + h, parent: current });
      }
    }
  }
  return [];
