...
export const ACHIEVEMENTS: Achievement[] = [
  ...
  { id: "thousand-pollen", title: "Rainha dos pólens", description: "Acumulou 1000 pólens", icon: "crown", rewardPollen: 100 },
...
export function useAchievements() {
  ...
  const persist = useCallback(async (set: Set<AchievementId>) => {
    try { await SecureStore.setItemAsync(STORAGE_KEY, JSON.stringify(Array.from(set))); } catch { /* ignora */ }
  }, []);
  ...
  const unlock = useCallback((id: AchievementId) => {
    setUnlocked((prev) => {
      ...
      if (ach) setPendingToast((prevToast) => [...prevToast, ach]);
      return next;
    });
  }, [persist]);

  const consumeToast = useCallback(() => setPendingToast((prev) => prev.slice(1)), []);
