...
export function usePassivePollen(map: BeeHouseMap, beePosition: () => Position) {
  ...
  const spawn = useCallback(() => {
    setItems((prev) => {
      ...
      for (let attempt = 0; attempt < 24; attempt++) {
        ...
        let amount: number;
        if (roll < 0.05) { type = "star"; amount = 10; }
        else if (roll < 0.7) { type = "pollen"; amount = 1 + Math.floor(Math.random() * 3); }
        else { type = "heart"; amount = 1; }
        return [...prev, { id: `passive-${nextItemId++}`, type, position: candidate, amount, spawnedAt: Date.now() }];
      }
      return prev;
    });
  }, [beePosition, map]);
  ...
  const collect = useCallback(
    (id: string): SpawnedItem | null => {
      ...
      if (target) setItems((prev) => prev.filter((i) => i.id !== id));
      return target;
    },
    [items],
  );
